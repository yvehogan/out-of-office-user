...
export function ShopCard({ title, image, bgColor, imagePosition, xlImagePosition }: ShopCardProps) {
  return (
    <div className={`group w-full min-h-[330px] p-6 gap-5 rounded-[30px] border border-transparent flex flex-col relative overflow-hidden transition-all duration-300 cursor-pointer hover:-translate-y-2 hover:shadow-[0_15px_35px_rgba(33,1,95,0.05)] xl:w-[280px] xl:h-[340px] 2xl:!w-[400px] 2xl:!h-[466px] min-[1600px]:h-[500px] xl:p-[24px] 2xl:!p-[20px_20px_72px_20px] xl:gap-0 ${bgColor}`}>
      <h3 className="relative z-10 font-ui text-[24px] font-semibold text-brand-purple2 self-start xl:text-[24px] 2xl:!text-[30px] tracking-tight">{title}</h3>
      
      {/* Unified Responsive Image Wrapper */}
      <div className={`absolute flex items-end justify-end pointer-events-auto xl:-bottom-2 xl:-right-2 2xl:bottom-auto 2xl:right-auto ${imagePosition} ${xlImagePosition ?? ''} xl:w-[85%] xl:h-[80%] 2xl:w-auto 2xl:h-auto`}>
        <img 
          src={image} 
          alt={title} 
